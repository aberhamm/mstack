...
- [x] Trigger collision resolved: "review the backlog" appears in BOTH
...
- [x] mstack-plan-multi line 347 (**"Never suggest 'all pending mstack plans
...
- [x] mstack-simplify-code/SKILL.md: the description says DEPRECATED/redirect
      ...
      Step 4b.
- [x] mstack-status "Update check" section (lines 65-77) is deleted: it probes
...
- [x] references/CONVENTION.md is refreshed: its Inventory table lists only
...
- [x] mstack-wrap-up lines 598-603 reference `PRE_DIRTY`/`MODIFIED` sets that
...
- [x] plan-template.md line 3 ("Copy this into `plans/NNN-slug.md`") says
...
- [x] The commit trailer at mstack-run SKILL.md line 984
      (`Refs: docs/plans/<file>`) becomes `Refs: <plans-dir>/<file>` so
      plans/-rooted repos get a correct trailer.
- [x] mstack-changelog line 20 ("You are running the `/changelog` skill")
      names itself `/mstack-changelog`.
- [x] mstack-stash save numbering (lines 70-73) derives `NEXT_NUM` from file
...
- [x] mstack-stash and mstack-handoff descriptions each gain a one-line "not
